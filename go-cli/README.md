# memesio-content-creation-cli

boru-driven command-line client **and** interactive REPL for the MemesioContentCreation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/memesio-content-creation-cli)
make build

# 2. See usage (words, entities, env vars)
./memesio-content-creation-cli --help

# 3. Provide credentials once, via the environment
export MEMESIO_CONTENT_CREATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./memesio-content-creation-cli load 1 agent            # {id:1} shorthand
./memesio-content-creation-cli load '{id:1}' agent       # explicit match map
./memesio-content-creation-cli update '{name:"x"}' agent

# 5. Override the API base URL for a single call
MEMESIO_CONTENT_CREATION_BASE=https://api.example.com ./memesio-content-creation-cli load 1 agent

# 6. No arguments -> interactive REPL
./memesio-content-creation-cli
memesio-content-creation> load 1 agent
memesio-content-creation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/memesio-content-creation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/memesio-content-creation-cli load 1 agent
   ```
...
```sh
./memesio-content-creation-cli load 1 agent          # scalar shorthand for {id:1}
./memesio-content-creation-cli load '{id:1}' agent     # explicit match map
```
...
```sh
./memesio-content-creation-cli update '{id:1,name:"new"}' agent
```
...
./memesio-content-creation-cli load 1 agent
```
...
Run with no arguments to open a REPL (prompt `memesio-content-creation>`). Each line is
evaluated as its own boru expression:

```text
$ ./memesio-content-creation-cli
memesio-content-creation> load 1 agent
memesio-content-creation> /help
memesio-content-creation> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/memesio-content-creation-cli`. |
...
(`./memesio-content-creation-cli <expr>`) and interactively (the REPL), and expressions compose the
