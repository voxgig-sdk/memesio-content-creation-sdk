# memesio-content-creation-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MemesioContentCreation SDK as
two agent tools — `memesio-content-creation_list` and `memesio-content-creation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/memesio-content-creation-mcp)
make build

# 2. Provide credentials via the environment
export MEMESIO_CONTENT_CREATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user memesio-content-creation \
  -- /absolute/path/to/memesio-content-creation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./memesio-content-creation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// memesio-content-creation_list: first page of records
{ "entity": "free_template_search" }
{ "entity": "free_template_search", "query": { } }

// memesio-content-creation_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/memesio-content-creation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user memesio-content-creation \
     -- "$PWD"/dist/*/memesio-content-creation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `memesio-content-creation_list` and `memesio-content-creation_load` tools now appear
   in new sessions. Ask the agent to *"list free_template_search using memesio-content-creation"*
   and it calls `memesio-content-creation_list` with `{"entity":"free_template_search"}`.
...
```sh
export MEMESIO_CONTENT_CREATION_APIKEY=sk_live_xxx            # API key
export MEMESIO_CONTENT_CREATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `memesio-content-creation_list` tool
...
### Call the `memesio-content-creation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `memesio-content-creation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `memesio-content-creation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MEMESIO_CONTENT_CREATION_APIKEY` | API key sent with every request. |
| `MEMESIO_CONTENT_CREATION_BASE` | Optional override of the API base URL. |
...
```sh
./memesio-content-creation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"memesio-content-creation_load","arguments":{"entity":"agent","query":{"id":1}}}}'
```
