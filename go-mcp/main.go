// Command MemesioContentCreation-mcp is an MCP server exposing the MemesioContentCreation
// SDK as MCP tools.
//
// Two tools are registered:
//
//	MemesioContentCreation_list  — list records of an entity
//	MemesioContentCreation_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: MEMESIO_CONTENT_CREATION_APIKEY carries the API key and
	// MEMESIO_CONTENT_CREATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MEMESIO_CONTENT_CREATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MEMESIO_CONTENT_CREATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMemesioContentCreationSDK(opts)
