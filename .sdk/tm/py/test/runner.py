# MemesioContentCreation SDK test runner
...
class MemesioContentCreationTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MemesioContentCreationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MemesioContentCreationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MemesioContentCreationTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = MemesioContentCreationTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MemesioContentCreationTestRunner.getenv(key)
        ...
        explain = MemesioContentCreationTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if MemesioContentCreationTestRunner._test_control is not None:
            return MemesioContentCreationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MemesioContentCreationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MemesioContentCreationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MemesioContentCreationTestRunner.load_test_control()
...
def load_env_local():
    MemesioContentCreationTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return MemesioContentCreationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MemesioContentCreationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MemesioContentCreationTestRunner.load_test_control()


def live_client_options():
    return MemesioContentCreationTestRunner.live_client_options()


def live_delay_ms():
    return MemesioContentCreationTestRunner.live_delay_ms()
