...
def make_error_util(ctx, err):
    if ctx is None:
        from projectname_sdk.core.context import MemesioContentCreationContext
        ctx = MemesioContentCreationContext({}, None)
    ...
    result = ctx.result
    if result is None:
        result = MemesioContentCreationResult({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, MemesioContentCreationError):
        errmsg = err.msg
    ...
    sdk_err = MemesioContentCreationError("", msg, ctx)
