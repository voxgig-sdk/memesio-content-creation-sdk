...
// MemesioContentCreation SDK feature test
//
...
class FtCtrl extends MemesioContentCreationControl
{
...
class FtClient
{
    ...
    private MemesioContentCreationUtility $utility;

    public function __construct(MemesioContentCreationUtility $utility, array $options)
    {
    ...
    public function get_utility(): MemesioContentCreationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MemesioContentCreationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MemesioContentCreationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MemesioContentCreationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MemesioContentCreationConfig::make_feature($name);
    ...
    private static function buildUrl(MemesioContentCreationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MemesioContentCreationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MemesioContentCreationResponse($fetched);
        }
    ...
    private function fail(MemesioContentCreationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MemesioContentCreationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
