...
// MemesioContentCreation SDK retry feature
...
class MemesioContentCreationRetryFeature extends MemesioContentCreationBaseFeature
{
    ...
    public function init(MemesioContentCreationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MemesioContentCreationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MemesioContentCreationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MemesioContentCreationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
