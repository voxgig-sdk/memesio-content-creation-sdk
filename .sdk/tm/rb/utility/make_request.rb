...
module MemesioContentCreationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MemesioContentCreationResponse.new({})
    ...
    if fetch_err
      ...
      response = MemesioContentCreationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MemesioContentCreationResponse.new(fetched)
    else
