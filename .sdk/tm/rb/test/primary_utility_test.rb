...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = MemesioContentCreationOmni.make_runner(TEST_JSON_FILE, MemesioContentCreationSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    MemesioContentCreationError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : MemesioContentCreationOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = MemesioContentCreationOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
