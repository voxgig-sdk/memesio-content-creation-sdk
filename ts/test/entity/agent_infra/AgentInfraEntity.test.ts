...
describe('AgentInfraEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'load', 'remove']) {
      if (maybeSkipControl(t, 'entityOp', 'agent_infra.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_AGENT_INFRA_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/agent_infra/AgentInfraTestData.json')
  ...
  let idmap = transform(
    ['agent_infra01','agent_infra02','agent_infra03','unlock01','unlock02','unlock03','agent01','agent02','agent03','agent01','agent02','agent03','key01','key02','key03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['MEMESIO_CONTENT_CREATION_TEST_AGENT_INFRA_ENTID']
