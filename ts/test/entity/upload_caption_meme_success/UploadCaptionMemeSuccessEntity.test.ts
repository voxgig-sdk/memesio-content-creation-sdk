...
describe('UploadCaptionMemeSuccessEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'upload_caption_meme_success.' + op, live)) return
    }
    ...
    upload_caption_meme_success_ref01_data = await upload_caption_meme_success_ref01_ent.create(upload_caption_meme_success_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['MEMESIO_CONTENT_CREATION_TEST_UPLOAD_CAPTION_MEME_SUCCESS_ENTID']
