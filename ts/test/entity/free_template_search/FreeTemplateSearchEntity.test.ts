...
describe('FreeTemplateSearchEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'free_template_search.' + op, live)) return
    }
    ...
    const free_template_search_ref01_list = await free_template_search_ref01_ent.list(free_template_search_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['MEMESIO_CONTENT_CREATION_TEST_FREE_TEMPLATE_SEARCH_ENTID']
