...
describe('AiMemeGenerationSucceededEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create']) {
      if (maybeSkipControl(t, 'entityOp', 'ai_meme_generation_succeeded.' + op, live)) return
    }
    ...
    ai_meme_generation_succeeded_ref01_data = await ai_meme_generation_succeeded_ref01_ent.create(ai_meme_generation_succeeded_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['MEMESIO_CONTENT_CREATION_TEST_AI_MEME_GENERATION_SUCCEEDED_ENTID']
