...
describe("AgentEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MEMESIOCONTENTCREATION_TEST_AGENT_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    agent_ref01_data = helpers.to_map(agent_ref01_data_result)
    ...
    assert.is_nil(err)
    local agent_ref01_resdata_up0 = helpers.to_map(agent_ref01_resdata_up0_result)
...
function agent_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MEMESIOCONTENTCREATION_TEST_AGENT_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["MEMESIOCONTENTCREATION_TEST_AGENT_ENTID"])
