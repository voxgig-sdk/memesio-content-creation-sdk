...
describe("MemeEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Meme(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = meme_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "meme." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_MEME_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local meme_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.meme")))
    local meme_ref01_data = nil
    if #meme_ref01_data_raw > 0 then
      meme_ref01_data = helpers.to_map(meme_ref01_data_raw[1][2])
    end
...
function meme_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/meme/MemeTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read meme test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("MEMESIO_CONTENT_CREATION_TEST_MEME_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["MEMESIO_CONTENT_CREATION_TEST_MEME_ENTID"])
