...
describe("AiJobEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set MEMESIOCONTENTCREATION_TEST_AI_JOB_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local ai_job_ref01_data_dt0_load_result = helpers.to_map(ai_job_ref01_data_dt0_loaded)
...
function ai_job_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("MEMESIOCONTENTCREATION_TEST_AI_JOB_ENTID")
