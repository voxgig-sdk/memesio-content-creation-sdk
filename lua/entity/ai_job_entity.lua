...
local AiJobEntity = {}
AiJobEntity.__index = AiJobEntity


function AiJobEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiJobEntity)
  self._name = "ai_job"
  self._client = client
...
function AiJobEntity:get_name()
  return self._name
end


function AiJobEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiJobEntity.new(self._client, opts)
end
...
function AiJobEntity:mark_deleted()
  self._deleted = true
end


function AiJobEntity:deleted()
  return true == self._deleted
end


function AiJobEntity:data_set(args)
  if args ~= nil then
...
function AiJobEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiJobEntity:match_set(args)
  if args ~= nil then
...
function AiJobEntity:match_get()
...
function AiJobEntity:stream(action, args, callopts)
...
---@param reqmatch AiJobLoadMatch
---@param ctrl? table
---@return AiJob
---@return string? err
function AiJobEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AiJobCreateData
---@param ctrl? table
---@return AiJob
---@return string? err
function AiJobEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AiJobEntity:_run_op(ctx, post_done)
  local utility = self._utility
