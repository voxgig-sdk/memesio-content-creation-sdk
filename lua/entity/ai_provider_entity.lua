...
local AiProviderEntity = {}
AiProviderEntity.__index = AiProviderEntity


function AiProviderEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiProviderEntity)
  self._name = "ai_provider"
  self._client = client
...
function AiProviderEntity:get_name()
  return self._name
end


function AiProviderEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiProviderEntity.new(self._client, opts)
end
...
function AiProviderEntity:mark_deleted()
  self._deleted = true
end


function AiProviderEntity:deleted()
  return true == self._deleted
end


function AiProviderEntity:data_set(args)
  if args ~= nil then
...
function AiProviderEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiProviderEntity:match_set(args)
  if args ~= nil then
...
function AiProviderEntity:match_get()
...
function AiProviderEntity:stream(action, args, callopts)
...
---@param reqmatch AiProviderLoadMatch
---@param ctrl? table
---@return AiProvider
---@return string? err
function AiProviderEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AiProviderCreateData
---@param ctrl? table
---@return AiProvider
---@return string? err
function AiProviderEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AiProviderEntity:_run_op(ctx, post_done)
  local utility = self._utility
