...
local AiCaptionEntity = {}
AiCaptionEntity.__index = AiCaptionEntity


function AiCaptionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiCaptionEntity)
  self._name = "ai_caption"
  self._client = client
...
function AiCaptionEntity:get_name()
  return self._name
end


function AiCaptionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiCaptionEntity.new(self._client, opts)
end
...
function AiCaptionEntity:mark_deleted()
  self._deleted = true
end


function AiCaptionEntity:deleted()
  return true == self._deleted
end


function AiCaptionEntity:data_set(args)
  if args ~= nil then
...
function AiCaptionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiCaptionEntity:match_set(args)
  if args ~= nil then
...
function AiCaptionEntity:match_get()
...
function AiCaptionEntity:stream(action, args, callopts)
...
---@param reqmatch AiCaptionLoadMatch
---@param ctrl? table
---@return AiCaption
---@return string? err
function AiCaptionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AiCaptionCreateData
---@param ctrl? table
---@return AiCaption
---@return string? err
function AiCaptionEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function AiCaptionEntity:_run_op(ctx, post_done)
  local utility = self._utility
