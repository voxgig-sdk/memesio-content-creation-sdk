-- MemesioContentCreation SDK CreateMeme entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CreateMemeEntity = {}
CreateMemeEntity.__index = CreateMemeEntity


function CreateMemeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CreateMemeEntity)
  self._name = "create_meme"
  self._client = client
...
function CreateMemeEntity:get_name()
  return self._name
end


function CreateMemeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CreateMemeEntity.new(self._client, opts)
end
...
function CreateMemeEntity:mark_deleted()
  self._deleted = true
end


function CreateMemeEntity:deleted()
  return true == self._deleted
end


function CreateMemeEntity:data_set(args)
  if args ~= nil then
...
function CreateMemeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CreateMemeEntity:match_set(args)
  if args ~= nil then
...
function CreateMemeEntity:match_get()
...
function CreateMemeEntity:stream(action, args, callopts)
...
---@param reqdata CreateMemeCreateData
---@param ctrl? table
---@return CreateMeme
---@return string? err
function CreateMemeEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function CreateMemeEntity:_run_op(ctx, post_done)
  local utility = self._utility
