-- MemesioContentCreation SDK FreeCaptionMemeSuccess entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local FreeCaptionMemeSuccessEntity = {}
FreeCaptionMemeSuccessEntity.__index = FreeCaptionMemeSuccessEntity


function FreeCaptionMemeSuccessEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FreeCaptionMemeSuccessEntity)
  self._name = "free_caption_meme_success"
  self._client = client
...
function FreeCaptionMemeSuccessEntity:get_name()
  return self._name
end


function FreeCaptionMemeSuccessEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FreeCaptionMemeSuccessEntity.new(self._client, opts)
end
...
function FreeCaptionMemeSuccessEntity:mark_deleted()
  self._deleted = true
end


function FreeCaptionMemeSuccessEntity:deleted()
  return true == self._deleted
end


function FreeCaptionMemeSuccessEntity:data_set(args)
  if args ~= nil then
...
function FreeCaptionMemeSuccessEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FreeCaptionMemeSuccessEntity:match_set(args)
  if args ~= nil then
...
function FreeCaptionMemeSuccessEntity:match_get()
...
function FreeCaptionMemeSuccessEntity:stream(action, args, callopts)
...
---@param reqdata FreeCaptionMemeSuccessCreateData
---@param ctrl? table
---@return FreeCaptionMemeSuccess
---@return string? err
function FreeCaptionMemeSuccessEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function FreeCaptionMemeSuccessEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return FreeCaptionMemeSuccessEntity
