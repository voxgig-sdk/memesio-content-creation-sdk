-- MemesioContentCreation SDK Compliance entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ComplianceEntity = {}
ComplianceEntity.__index = ComplianceEntity


function ComplianceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ComplianceEntity)
  self._name = "compliance"
  self._client = client
...
function ComplianceEntity:get_name()
  return self._name
end


function ComplianceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ComplianceEntity.new(self._client, opts)
end
...
function ComplianceEntity:mark_deleted()
  self._deleted = true
end


function ComplianceEntity:deleted()
  return true == self._deleted
end


function ComplianceEntity:data_set(args)
  if args ~= nil then
...
function ComplianceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ComplianceEntity:match_set(args)
  if args ~= nil then
...
function ComplianceEntity:match_get()
...
function ComplianceEntity:stream(action, args, callopts)
...
---@param reqmatch ComplianceLoadMatch
---@param ctrl? table
---@return Compliance
---@return string? err
function ComplianceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ComplianceEntity:_run_op(ctx, post_done)
  local utility = self._utility
