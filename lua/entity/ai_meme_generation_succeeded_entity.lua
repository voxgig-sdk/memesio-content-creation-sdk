-- MemesioContentCreation SDK AiMemeGenerationSucceeded entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AiMemeGenerationSucceededEntity = {}
AiMemeGenerationSucceededEntity.__index = AiMemeGenerationSucceededEntity


function AiMemeGenerationSucceededEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AiMemeGenerationSucceededEntity)
  self._name = "ai_meme_generation_succeeded"
  self._client = client
...
function AiMemeGenerationSucceededEntity:get_name()
  return self._name
end


function AiMemeGenerationSucceededEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AiMemeGenerationSucceededEntity.new(self._client, opts)
end
...
function AiMemeGenerationSucceededEntity:mark_deleted()
  self._deleted = true
end


function AiMemeGenerationSucceededEntity:deleted()
  return true == self._deleted
end


function AiMemeGenerationSucceededEntity:data_set(args)
  if args ~= nil then
...
function AiMemeGenerationSucceededEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AiMemeGenerationSucceededEntity:match_set(args)
  if args ~= nil then
...
function AiMemeGenerationSucceededEntity:match_get()
...
function AiMemeGenerationSucceededEntity:stream(action, args, callopts)
...
---@param reqdata AiMemeGenerationSucceededCreateData
---@param ctrl? table
---@return AiMemeGenerationSucceeded
---@return string? err
function AiMemeGenerationSucceededEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function AiMemeGenerationSucceededEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AiMemeGenerationSucceededEntity
