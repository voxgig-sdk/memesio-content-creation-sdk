    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MEMESIOCONTENTCREATION_TEST_COLLABORATION_ENTID JSON to run live")
        client = setup["client"]
        ...
        collaboration_ref01_data = helpers.to_map(collaboration_ref01_ent.create(collaboration_ref01_data, None))
        assert collaboration_ref01_data is not None

        # LOAD
        collaboration_ref01_match_dt0 = {}
        collaboration_ref01_data_dt0_loaded = collaboration_ref01_ent.load(collaboration_ref01_match_dt0, None)
        assert collaboration_ref01_data_dt0_loaded is not None
...
def _collaboration_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "MEMESIOCONTENTCREATION_TEST_COLLABORATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MEMESIOCONTENTCREATION_TEST_COLLABORATION_ENTID"))
