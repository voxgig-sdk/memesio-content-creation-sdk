...
class TestTemplateSearchDirect:

    def test_should_direct_list_template_search(self):
        setup = _template_search_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-template_search", "live" if setup["live"] else "unit")
        if _skip:
...
def _template_search_direct_setup(mockres):
