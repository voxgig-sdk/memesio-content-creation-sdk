...
class TestAgentInfraDirect:

    def test_should_direct_load_agent_infra(self):
        setup = _agent_infra_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-agent_infra", "live" if setup["live"] else "unit")
        if _skip:
...
def _agent_infra_direct_setup(mockres):
