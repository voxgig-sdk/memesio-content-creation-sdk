...
class TestListMemeDirect:

    def test_should_direct_list_list_meme(self):
        setup = _list_meme_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-list_meme", "live" if setup["live"] else "unit")
        if _skip:
...
def _list_meme_direct_setup(mockres):
