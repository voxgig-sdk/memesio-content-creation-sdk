...
class TestGrowthDirect:

    def test_should_direct_load_growth(self):
        setup = _growth_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-growth", "live" if setup["live"] else "unit")
        if _skip:
...
def _growth_direct_setup(mockres):
