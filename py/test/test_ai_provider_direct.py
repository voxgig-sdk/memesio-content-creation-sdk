...
class TestAiProviderDirect:

    def test_should_direct_load_ai_provider(self):
        setup = _ai_provider_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ai_provider", "live" if setup["live"] else "unit")
        if _skip:
...
def _ai_provider_direct_setup(mockres):
