    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MEMESIOCONTENTCREATION_TEST_STANDALONE_AGENT_BOOTSTRAP_ENTID JSON to run live")
        client = setup["client"]
        ...
        standalone_agent_bootstrap_ref01_data = helpers.to_map(standalone_agent_bootstrap_ref01_ent.create(standalone_agent_bootstrap_ref01_data, None))
...
def _standalone_agent_bootstrap_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "MEMESIOCONTENTCREATION_TEST_STANDALONE_AGENT_BOOTSTRAP_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MEMESIOCONTENTCREATION_TEST_STANDALONE_AGENT_BOOTSTRAP_ENTID"))
