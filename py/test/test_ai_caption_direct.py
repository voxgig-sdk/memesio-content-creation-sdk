...
class TestAiCaptionDirect:

    def test_should_direct_load_ai_caption(self):
        setup = _ai_caption_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ai_caption", "live" if setup["live"] else "unit")
        if _skip:
...
def _ai_caption_direct_setup(mockres):
