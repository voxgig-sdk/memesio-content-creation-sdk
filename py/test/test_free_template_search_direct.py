...
class TestFreeTemplateSearchDirect:

    def test_should_direct_list_free_template_search(self):
        setup = _free_template_search_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-free_template_search", "live" if setup["live"] else "unit")
        if _skip:
...
def _free_template_search_direct_setup(mockres):
