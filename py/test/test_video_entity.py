    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MEMESIO_CONTENT_CREATION_TEST_VIDEO_ENTID JSON to run live")
        client = setup["client"]
        ...
        video_ref01_data = helpers.to_map(runner.entity_data(video_ref01_ent.create(video_ref01_data, None)))
        assert video_ref01_data is not None
...
def _video_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "MEMESIO_CONTENT_CREATION_TEST_VIDEO_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MEMESIO_CONTENT_CREATION_TEST_VIDEO_ENTID"))
