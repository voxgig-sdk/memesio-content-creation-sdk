...
class TestTrendAlertDirect:

    def test_should_direct_load_trend_alert(self):
        setup = _trend_alert_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-trend_alert", "live" if setup["live"] else "unit")
        if _skip:
...
def _trend_alert_direct_setup(mockres):
