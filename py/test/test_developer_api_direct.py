...
class TestDeveloperApiDirect:

    def test_should_direct_load_developer_api(self):
        setup = _developer_api_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-developer_api", "live" if setup["live"] else "unit")
        if _skip:
...
def _developer_api_direct_setup(mockres):
