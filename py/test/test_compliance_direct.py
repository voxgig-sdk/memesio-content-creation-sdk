...
class TestComplianceDirect:

    def test_should_direct_load_compliance(self):
        setup = _compliance_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-compliance", "live" if setup["live"] else "unit")
        if _skip:
...
def _compliance_direct_setup(mockres):
