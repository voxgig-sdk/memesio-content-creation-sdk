...
class TestMemeDirect:

    def test_should_direct_load_meme(self):
        setup = _meme_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-meme", "live" if setup["live"] else "unit")
        if _skip:
...
def _meme_direct_setup(mockres):
