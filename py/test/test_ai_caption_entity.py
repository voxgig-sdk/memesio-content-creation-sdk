...
class TestAiCaptionEntity:

    def test_should_create_instance(self):
        testsdk = MemesioContentCreationSDK.test(None, None)
        ent = testsdk.AiCaption(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _ai_caption_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "ai_caption." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MEMESIO_CONTENT_CREATION_TEST_AI_CAPTION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        ai_caption_ref01_ent = client.AiCaption(None)
        ai_caption_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.ai_caption"), "ai_caption_ref01"))

        ai_caption_ref01_data = helpers.to_map(runner.entity_data(ai_caption_ref01_ent.create(ai_caption_ref01_data, None)))
        assert ai_caption_ref01_data is not None

        # LOAD
        ai_caption_ref01_match_dt0 = {}
        ai_caption_ref01_data_dt0_loaded = ai_caption_ref01_ent.load(ai_caption_ref01_match_dt0, None)
        assert ai_caption_ref01_data_dt0_loaded is not None



def _ai_caption_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/ai_caption/AiCaptionTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MEMESIO_CONTENT_CREATION_TEST_AI_CAPTION_ENTID"))
