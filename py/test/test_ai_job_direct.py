...
class TestAiJobDirect:

    def test_should_direct_load_ai_job(self):
        setup = _ai_job_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-ai_job", "live" if setup["live"] else "unit")
        if _skip:
...
def _ai_job_direct_setup(mockres):
