...
from memesiocontentcreation_types import (
    AgentInfra,
    AgentInfraLoadMatch,
...
class AgentInfraEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AgentInfraEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> AgentInfra:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.AgentInfra().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: AgentInfraCreateData, ctrl=None) -> AgentInfra:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> AgentInfra:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.AgentInfra().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
