...
class DeveloperApiEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DeveloperApiEntity(self._client, opts)
    ...
    def data_get(self) -> DeveloperApi:
    ...
    def match_get(self) -> DeveloperApi:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DeveloperApi:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DeveloperApi().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DeveloperApiCreateData, ctrl=None) -> DeveloperApi:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
