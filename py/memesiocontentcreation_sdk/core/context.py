...
from memesiocontentcreation_sdk.core.helpers import get_ctx_prop, to_map
...
class MemesioContentCreationContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = MemesioContentCreationControl()
        ...
        if isinstance(sp, MemesioContentCreationSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, MemesioContentCreationResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, MemesioContentCreationResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return MemesioContentCreationOperation({})
        ...
        op = MemesioContentCreationOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return MemesioContentCreationError(code, msg, self)
