...
// MemesioContentCreation SDK utility: result_basic

class MemesioContentCreationResultBasic
{
    public static function call(MemesioContentCreationContext $ctx): ?MemesioContentCreationResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof MemesioContentCreationError) ? $result->err->msg : (string)$result->err;
