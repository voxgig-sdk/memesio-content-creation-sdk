...
// MemesioContentCreation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = MemesioContentCreationOmni::makeStructRunner(
    ...
    {
        return MemesioContentCreationOmni::objmodel(MemesioContentCreationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return MemesioContentCreationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return MemesioContentCreationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(MemesioContentCreationOmni::structfix($now, Struct::undef()))
                        !== json_encode(MemesioContentCreationOmni::structfix($val, Struct::undef()))) {
