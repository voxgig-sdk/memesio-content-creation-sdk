...
class VideoEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_VIDEO_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $video_ref01_data_result = $video_ref01_ent->create($video_ref01_data, null);
        $video_ref01_data = Helpers::to_map(is_object($video_ref01_data_result) && method_exists($video_ref01_data_result, 'data_get') ? $video_ref01_data_result->data_get() : $video_ref01_data_result);
        $this->assertNotNull($video_ref01_data);
...
function video_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MEMESIO_CONTENT_CREATION_TEST_VIDEO_ENTID");
