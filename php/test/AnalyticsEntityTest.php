...
class AnalyticsEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_ANALYTICS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function analytics_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MEMESIO_CONTENT_CREATION_TEST_ANALYTICS_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["MEMESIO_CONTENT_CREATION_TEST_ANALYTICS_ENTID"]);
