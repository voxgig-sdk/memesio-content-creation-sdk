...
class AgentInfraEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = MemesioContentCreationSDK::test(null, null);
        $ent = $testsdk->AgentInfra(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = agent_infra_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "agent_infra." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $agent_infra_ref01_ent = $client->AgentInfra(null);
...
function agent_infra_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/agent_infra/AgentInfraTestData.json';
