...
class AgentInfraEntity
  ...
  def make
    opts = @_entopts.dup
    AgentInfraEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MemesioContentCreationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [AgentInfra, Hash] the current AgentInfra data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MemesioContentCreationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of AgentInfra fields)
  def match_get
  ...
  #
  # @param reqmatch [AgentInfraLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.AgentInfra.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [AgentInfra, Hash] the loaded AgentInfra; raises MemesioContentCreationError on failure
  ...
  #
  # @param reqdata [AgentInfraCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [AgentInfra, Hash] the created AgentInfra; raises MemesioContentCreationError on failure
  ...
  #
  # @param reqmatch [AgentInfraRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [AgentInfra, Hash] the removed AgentInfra; raises MemesioContentCreationError on failure
