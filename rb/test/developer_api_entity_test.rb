...
class DeveloperApiEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_DEVELOPER_API_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    developer_api_ref01_data = Helpers.to_map(developer_api_ref01_data_result.respond_to?(:data_get) ? developer_api_ref01_data_result.data_get : developer_api_ref01_data_result)
...
def developer_api_basic_setup(extra)
  ...
  entid_env_raw = ENV["MEMESIO_CONTENT_CREATION_TEST_DEVELOPER_API_ENTID"]
