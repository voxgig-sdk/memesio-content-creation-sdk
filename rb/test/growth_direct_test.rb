...
class GrowthDirectTest < Minitest::Test
  def test_direct_load_growth
    setup = growth_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-growth", setup[:live] ? "live" : "unit")
    if _should_skip
...
def growth_direct_setup(mockres)
