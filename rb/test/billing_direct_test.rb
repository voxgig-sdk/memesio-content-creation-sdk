...
class BillingDirectTest < Minitest::Test
  def test_direct_load_billing
    setup = billing_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-billing", setup[:live] ? "live" : "unit")
    if _should_skip
...
def billing_direct_setup(mockres)
