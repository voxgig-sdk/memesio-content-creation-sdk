...
class TemplateSearchEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.TemplateSearch(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.TemplateSearch(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = template_search_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "template_search." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MEMESIOCONTENTCREATION_TEST_TEMPLATE_SEARCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    template_search_ref01_data = nil
    if template_search_ref01_data_raw.length > 0
      template_search_ref01_data = Helpers.to_map(template_search_ref01_data_raw[0][1])
    end

    # LIST
    template_search_ref01_ent = client.TemplateSearch(nil)
    template_search_ref01_match = {}

    template_search_ref01_list_result = template_search_ref01_ent.list(template_search_ref01_match, nil)
    assert template_search_ref01_list_result.is_a?(Array)

  end
end

def template_search_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "template_search", "TemplateSearchTestData.json")
  ...
  entid_env_raw = ENV["MEMESIOCONTENTCREATION_TEST_TEMPLATE_SEARCH_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["MEMESIOCONTENTCREATION_TEST_TEMPLATE_SEARCH_ENTID"])
