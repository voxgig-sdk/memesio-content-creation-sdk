...
class TemplateSearchDirectTest < Minitest::Test
  def test_direct_list_template_search
    setup = template_search_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-template_search", setup[:live] ? "live" : "unit")
    if _should_skip
...
def template_search_direct_setup(mockres)
