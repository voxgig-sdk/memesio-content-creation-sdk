...
class FreeTemplateSearchDirectTest < Minitest::Test
  def test_direct_list_free_template_search
    setup = free_template_search_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-free_template_search", setup[:live] ? "live" : "unit")
    if _should_skip
...
def free_template_search_direct_setup(mockres)
