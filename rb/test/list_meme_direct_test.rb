...
class ListMemeDirectTest < Minitest::Test
  def test_direct_list_list_meme
    setup = list_meme_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-list_meme", setup[:live] ? "live" : "unit")
    if _should_skip
...
def list_meme_direct_setup(mockres)
