...
class VideoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_VIDEO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    video_ref01_data = Helpers.to_map(video_ref01_data_result.respond_to?(:data_get) ? video_ref01_data_result.data_get : video_ref01_data_result)
...
def video_basic_setup(extra)
  ...
  entid_env_raw = ENV["MEMESIO_CONTENT_CREATION_TEST_VIDEO_ENTID"]
