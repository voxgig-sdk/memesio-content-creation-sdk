...
class StandaloneAgentBootstrapEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_STANDALONE_AGENT_BOOTSTRAP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    standalone_agent_bootstrap_ref01_data = Helpers.to_map(standalone_agent_bootstrap_ref01_data_result.respond_to?(:data_get) ? standalone_agent_bootstrap_ref01_data_result.data_get : standalone_agent_bootstrap_ref01_data_result)
...
def standalone_agent_bootstrap_basic_setup(extra)
  ...
  entid_env_raw = ENV["MEMESIO_CONTENT_CREATION_TEST_STANDALONE_AGENT_BOOTSTRAP_ENTID"]
