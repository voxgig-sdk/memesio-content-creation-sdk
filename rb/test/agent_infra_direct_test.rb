...
class AgentInfraDirectTest < Minitest::Test
  def test_direct_load_agent_infra
    setup = agent_infra_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-agent_infra", setup[:live] ? "live" : "unit")
    if _should_skip
...
def agent_infra_direct_setup(mockres)
