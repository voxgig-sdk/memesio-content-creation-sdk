...
class MemeDirectTest < Minitest::Test
  def test_direct_load_meme
    setup = meme_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-meme", setup[:live] ? "live" : "unit")
    if _should_skip
