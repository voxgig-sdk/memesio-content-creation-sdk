...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MEMESIOCONTENTCREATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("MEMESIOCONTENTCREATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MEMESIOCONTENTCREATION_TEST_EXPLAIN"); explain != "" {
