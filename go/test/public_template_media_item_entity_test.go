...
func TestPublicTemplateMediaItemEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.PublicTemplateMediaItem(nil)
		if ent == nil {
			t.Fatal("expected non-nil PublicTemplateMediaItemEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := public_template_media_itemBasicSetup(nil)
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MEMESIOCONTENTCREATION_TEST_PUBLIC_TEMPLATE_MEDIA_ITEM_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		publicTemplateMediaItemRef01DataDt0Loaded, err := publicTemplateMediaItemRef01Ent.Load(publicTemplateMediaItemRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		publicTemplateMediaItemRef01DataDt0LoadResult := core.ToMapAny(publicTemplateMediaItemRef01DataDt0Loaded)
		if publicTemplateMediaItemRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if publicTemplateMediaItemRef01DataDt0LoadResult["id"] != publicTemplateMediaItemRef01Data["id"] {
...
func public_template_media_itemBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "public_template_media_item", "PublicTemplateMediaItemTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read public_template_media_item test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse public_template_media_item test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("MEMESIOCONTENTCREATION_TEST_PUBLIC_TEMPLATE_MEDIA_ITEM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MEMESIOCONTENTCREATION_TEST_PUBLIC_TEMPLATE_MEDIA_ITEM_ENTID"])
