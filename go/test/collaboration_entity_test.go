...
func TestCollaborationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MEMESIOCONTENTCREATION_TEST_COLLABORATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		collaborationRef01Data = core.ToMapAny(collaborationRef01DataResult)
...
func collaborationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MEMESIOCONTENTCREATION_TEST_COLLABORATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MEMESIOCONTENTCREATION_TEST_COLLABORATION_ENTID"])
