...
func TestTemplateEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MEMESIOCONTENTCREATION_TEST_TEMPLATE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func templateBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MEMESIOCONTENTCREATION_TEST_TEMPLATE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MEMESIOCONTENTCREATION_TEST_TEMPLATE_ENTID"])
