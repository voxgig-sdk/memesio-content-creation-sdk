...
func TestUploadCaptionMemeSuccessEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_UPLOAD_CAPTION_MEME_SUCCESS_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		uploadCaptionMemeSuccessRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "upload_caption_meme_success"}), "upload_caption_meme_success_ref01"))
		...
		uploadCaptionMemeSuccessRef01Data = core.ToMapAny(entityData(uploadCaptionMemeSuccessRef01DataResult))
...
func upload_caption_meme_successBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MEMESIO_CONTENT_CREATION_TEST_UPLOAD_CAPTION_MEME_SUCCESS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MEMESIO_CONTENT_CREATION_TEST_UPLOAD_CAPTION_MEME_SUCCESS_ENTID"])
