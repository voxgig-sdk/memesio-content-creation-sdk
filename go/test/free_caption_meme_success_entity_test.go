...
func TestFreeCaptionMemeSuccessEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MEMESIOCONTENTCREATION_TEST_FREE_CAPTION_MEME_SUCCESS_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		freeCaptionMemeSuccessRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "free_caption_meme_success"}, setup.data), "free_caption_meme_success_ref01"))

		freeCaptionMemeSuccessRef01DataResult, err := freeCaptionMemeSuccessRef01Ent.Create(freeCaptionMemeSuccessRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		freeCaptionMemeSuccessRef01Data = core.ToMapAny(freeCaptionMemeSuccessRef01DataResult)
...
func free_caption_meme_successBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MEMESIOCONTENTCREATION_TEST_FREE_CAPTION_MEME_SUCCESS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MEMESIOCONTENTCREATION_TEST_FREE_CAPTION_MEME_SUCCESS_ENTID"])
