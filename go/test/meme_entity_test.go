...
func TestMemeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MEMESIO_CONTENT_CREATION_TEST_MEME_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		memeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.meme")))
...
func memeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MEMESIO_CONTENT_CREATION_TEST_MEME_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MEMESIO_CONTENT_CREATION_TEST_MEME_ENTID"])
