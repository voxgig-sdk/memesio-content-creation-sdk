...
func TestStandaloneAgentBootstrapEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MEMESIOCONTENTCREATION_TEST_STANDALONE_AGENT_BOOTSTRAP_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		standaloneAgentBootstrapRef01Data = core.ToMapAny(standaloneAgentBootstrapRef01DataResult)
...
func standalone_agent_bootstrapBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MEMESIOCONTENTCREATION_TEST_STANDALONE_AGENT_BOOTSTRAP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MEMESIOCONTENTCREATION_TEST_STANDALONE_AGENT_BOOTSTRAP_ENTID"])
