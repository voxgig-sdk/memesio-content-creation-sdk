...
func TestGrowthDirect(t *testing.T) {
	t.Run("direct-load-growth", func(t *testing.T) {
		setup := growthDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-growth", _mode); _shouldSkip {
			if _reason == "" {
...
type growthDirectSetupResult struct {
...
func growthDirectSetup(mockres any) *growthDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &growthDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &growthDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
