...
import (
	...
	omni "github.com/voxgig-sdk/memesio-content-creation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MemesioContentCreationError); is {
				out := map[string]any{
					"name":    "MemesioContentCreationError",
...
func omniCtx(arg any, client *sdk.MemesioContentCreationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MemesioContentCreationSDK); is {
			client = live
